...
def run_astar():
    ...
    if start == "" or goal == "":
        messagebox.showerror("Input Error", "Enter both Source+Dest")
        return
    ...
    if start == goal:
        messagebox.showerror("Input Error", "Source and Destination cannot be same.")
        return
    ...
    result_text.insert(tk.END, "Visited Nodes:\n")
...
tk.Label(root, text="Enter Destination City").pack()
goal_entry = tk.Entry(root)
goal_entry.pack()

tk.Button(root, text="Run A*", command=run_astar).pack(pady=10)

tk.Button(root, text="Show Complete Graph", command=show_graph).pack(pady=10)
